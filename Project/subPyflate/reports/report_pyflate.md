...
- All claims are grounded in project artifacts under `Project/subPyflate/docs/`,
  `Project/subPyflate/results/`, `Project/subPyflate/profiling/`, and
...
- Placeholders labeled `TBD` mark values collected on the VM after this
  ...
  script (see `Project/subPyflate/scripts/script_pyflate.sh`).

Primary intended outcome:
- Best software version selected: `Project/subPyflate/optimized/final/` (copy of
  Attempt 3).
...
- SystemVerilog under `Project/subPyflate/hw/rtl/`.
- SystemVerilog testbenches under `Project/subPyflate/hw/tb/`.
...
- `Project/subPyflate/results/baseline/stdout.txt` contains the pyperformance mean
  / std-dev line.
- `Project/subPyflate/reports/baseline_results.txt` is the human summary.
- Mean: `TBD` s. Std dev: `TBD` s.

`Project/subRay/reports/baseline_results.txt` recorded 26.8 s ± 5.7 s with
...
Project/subPyflate/original/bm_pyflate/run_benchmark.py --loops=1`. Run directly
...
Selected: `Project/subPyflate/optimized/final/` — default copy of Attempt 3. If VM
...
- Original: `Project/subPyflate/results/original_official.txt`
- Final: `Project/subPyflate/results/final_official.txt`
