#!/usr/bin/env python
"""
Optimization Attempt 3 for the pyflate benchmark.

Adds on top of Attempt 2:
  * `decode_huffman_block` hoists every hot attribute/method lookup into a
    local name (Python attribute reads cost real time in a tight loop). This
    includes `field.readbits`, `t.find_next_symbol`, `favourites.pop`,
    `favourites.insert`, `buffer.append`, and the `_INT2BYTE` global.
  * The end-of-block RLE decode loop uses direct integer indexing into a
    single `memoryview` instead of Python-level slicing/`ord()`, which drops
    per-byte allocation for long runs.

Everything else is inherited from Attempt 2 (LUT + MTF/BWT/int2byte).
"""
...
# Precomputed byte-value -> single-byte bytes-object lookup. Replacing
# `int2byte(i)` (a `struct.Struct(">B").pack` call) with `_INT2BYTE[i]`
# removes per-call C-side setup for every literal byte we emit.
_INT2BYTE = tuple(bytes((i,)) for i in range(256))
...
class HuffmanTable(object):
    ...
    def _build_lut(self, reversed_):
        """Build a canonical Huffman lookup table for one direction."""
        if not hasattr(self, "max_bits") or self.max_bits < 0:
            self.min_max_bits()
        max_bits = self.max_bits
        lut = [(-1, 0)] * (1 << max_bits) if max_bits > 0 else []
        for x in self.table:
            if x.bits <= 0 or x.bits > max_bits:
                continue
            pad = max_bits - x.bits
            code_bits = x.bits
            entry = (x.code, code_bits)
            if reversed_:
                # Bitfield (LSB-first): bits are the low bits, suffix goes high.
                base = x.reverse_symbol
                step = 1 << code_bits
                idx = base
                for _ in range(1 << pad):
                    lut[idx] = entry
                    idx += step
            else:
                # RBitfield (MSB-first): bits are the high bits, suffix goes low.
                base = x.symbol << pad
                for suffix in range(1 << pad):
                    lut[base | suffix] = entry
        return lut
    ...
    def _fallback_scan(self, field, reversed_):
        cached_length = -1
        cached = None
        for x in self.table:
            ...
            if (reversed_ and x.reverse_symbol == cached) or (not reversed_ and x.symbol == cached):
    ...
    def find_next_symbol(self, field, reversed=True):
        ...
        if lut is None:
            lut = self._build_lut(reversed)
            setattr(self, attr, lut)
        try:
            code, bits = lut[field.snoopbits(self.max_bits)]
        except Exception:
            return self._fallback_scan(field, reversed)
        if code < 0:
            return self._fallback_scan(field, reversed)
        field.readbits(bits)
        return code
...
def move_to_front(l, c):
    # Original: `l[:] = l[c:c + 1] + l[0:c] + l[c + 1:]` (three slice allocs).
    # Equivalent: pop the c'th element and re-insert at index 0.
    if c:
        l.insert(0, l.pop(c))


def bwt_transform(L):
    # Original: `F = bytes(sorted(L))` then 256 `F.find(int2byte(i))` scans.
    # Equivalent: single-pass frequency count + cumulative sum gives the same
    # `base[i]` = index of the first occurrence of byte i in sorted(L).
    counts = [0] * 256
    for c in L:
        counts[c] += 1
    base = [0] * 256
...
def decode_huffman_block(b, out):
    ...
    favourites = [_INT2BYTE[i] for i, x in enumerate(used) if x]
    ...
    favourites_pop = favourites.pop
    favourites_insert = favourites.insert
    n_selectors = len(selectors_list)
    end_symbol = symbols_in_use - 1
    t = None
    t_find = None
    while True:
        decoded -= 1
        if decoded <= 0:
            decoded = 50
            if selector_pointer <= n_selectors:
                t = tables[selectors_list[selector_pointer]]
                t_find = t.find_next_symbol
                selector_pointer += 1
        ...
        if 0 <= r <= 1:
            ...
            buffer_append(favourites[0] * repeat)
            repeat = 0
        if r == end_symbol:
            break
        else:
    ...
    # Byte-level RLE decode: identical semantics to the original, but works
    # directly on a memoryview and precomputed `_INT2BYTE` table, so we no
    # longer build one-byte `bytes` slices per iteration.
    mv = nearly_there
    n = len(mv)
    tbl = _INT2BYTE
    ...
    while i < n:
        if i < end and mv[i] == mv[i + 1] == mv[i + 2] == mv[i + 3]:
            out_append(tbl[mv[i]] * (mv[i + 4] + 4))
            i += 5
        else:
            out_append(tbl[mv[i]])
            i += 1
    ...
    while True:
        ...
        if blocktype == 0:
            ...
            for i in range(length):
                out.append(_INT2BYTE[b.readbits(8)])
...
if __name__ == '__main__':
    ...
    runner.metadata['description'] = "Pyflate benchmark (Attempt 3: LUT + MTF/BWT/int2byte + hoisting)"

    # Use the same data file as the pyperformance benchmark ships with. This
    # attempt is discovered by pyperformance through the sibling MANIFEST /
    # pyproject.toml but can also be run directly for correctness checks:
    #
    #   python3-dbg subPyflate/optimized/attempt1/run_benchmark.py --loops=1
    default_data = os.path.join(
        os.path.dirname(os.path.abspath(__file__)),
        "..", "..", "original", "bm_pyflate", "data", "interpreter.tar.bz2")
    default_data = os.path.normpath(default_data)
    ...
    if not os.path.exists(default_data):
        try:
            ...
            pkg_data = os.path.join(
                ...
                "data-files", "benchmarks", "bm_pyflate", "data",
                "interpreter.tar.bz2")
            if os.path.exists(pkg_data):
                default_data = pkg_data
        except Exception:
            pass

    runner.bench_time_func('pyflate_attempt3', bench_pyflake, default_data)
