...
- Use `Project/subRay/` as the root working directory.
...
- First create `Project/subRay/docs/00_project_plan.md` and `Project/subRay/docs/00_project_plan.docx`, with all requirements represented by `[ ]`, `[~]`, or `[x]` status markers.
...
1. Create `Project/subRay/docs/00_project_plan.md` with all 19 requirements marked `[ ]` Not started.
...
- `Project/subRay/docs/00_project_plan.md` was created with all 19 requirements marked `[ ]` Not started.
- `pandoc` was not available (`pandoc command not found`), so neither DOCX counterpart could be generated.
- The raw tool check is preserved in `Project/subRay/logs/00_pandoc_check.txt`.
...
- Create `Project/subRay/docs/01_benchmark_understanding.md` before analysis and later create its DOCX counterpart.
...
- Copy untouched relevant sources to `Project/subRay/original/` and calculate SHA256 checksums.
...
Completed in `Project/subRay/docs/01_benchmark_understanding.md`. The VM benchmark was resolved as `raytrace`; its untouched source and metadata were preserved under `Project/subRay/original/bm_raytrace/` with matching SHA256 checksums. No benchmark workload was executed and no source was modified.

DOCX generation was attempted after completing the Markdown but failed because `pandoc` is not installed. The error is preserved in `Project/subRay/logs/01_pandoc_check.txt`.
...
DOCX generation is dropped from the workflow. The originals under `Project/subRay/original/bm_raytrace/` were re-verified after external edits and still match the VM SHA256 hashes exactly (`88ef4d90...` and `b003d38f...`). No benchmark code was modified.
...
- Create `Project/subRay/docs/02_pyperformance.md` before running experiments.
...
- Save every command executed and raw outputs under `Project/subRay/logs/`.
...
The interactive QEMU serial handle was unavailable this turn. On the user's instruction, the step was completed using only VM output already captured earlier today (`Project/subRay/logs/02_pyperformance_env.txt`). Verified: pyperformance `1.14.0`, benchmark id `raytrace`, interpreter minor `3.10`, `/usr/bin/python3` launcher, and the list/run commands. Recorded as unverified rather than guessed: python3 exact patch version and python3-dbg availability. All measurement concepts were documented. No benchmark run, no optimization, no source change, no DOCX.
...
- Create `Project/subRay/docs/03_baseline.md` before benchmarking.
- Create `Project/subRay/scripts/run_baseline.sh`.
...
- Save raw output under `Project/subRay/results/baseline/` and save a summarized result under `Project/subRay/reports/baseline_results.txt`.
...
- Create `Project/subRay/docs/04_perf_profiling.md`.
...
- Save `Project/subRay/profiling/perf.data` and generate `Project/subRay/profiling/perf_report.txt` using `perf report --stdio`.
