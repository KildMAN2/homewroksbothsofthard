...
**Created:** 2026-09-13
**Status:** In progress
**Constraint:** `Project/subRay/original/` remains unchanged.
...
- **Exact planned modification:** Copy the original benchmark to `Project/subRay/optimized/attempt1/` and rewrite the hottest vector/point/ray arithmetic path to use scalar local floats and tuple-based scene data instead of allocating many `Vector`, `Point`, and `Ray` objects in the inner render and shading loops.
- **Files to change:**
  - `Project/subRay/optimized/attempt1/run_benchmark.py`
  - `Project/subRay/optimized/attempt1/pyproject.toml`
  - helper validation/benchmark scripts under `Project/subRay/scripts/` if needed for reproducible testing
...
1. Keep `Project/subRay/original/` untouched.
...
4. Record both the command and the result under `Project/subRay/results/attempt1/`.
...
3. Save stdout, stderr, environment data, and timing results under `Project/subRay/results/attempt1/`.
4. Summarize the measured comparison in `Project/subRay/reports/attempt1_results.txt`.
