...
3. Generate intermediate stack data under `Project/subRay/profiling/flamegraph_data/`.
4. Render the final SVG to `Project/subRay/profiling/flamegraph.svg`.
5. Preserve command outputs and errors under `Project/subRay/logs/`.
...
- `Project/subRay/profiling/perf.data`
- `Project/subRay/profiling/perf_report.txt`
- `Project/subRay/profiling/flamegraph.svg`
- `Project/subRay/profiling/flamegraph_data/`
- `Project/subRay/logs/04_perf_report_stderr.txt`
- `Project/subRay/logs/04_perf_step.txt`
- `Project/subRay/logs/04_flamegraph_step.txt`
...
```bash
perf record -F 999 -g -o /root/homewroksbothsofthard/Project/subRay/results/baseline/perf.data -- python3-dbg -m pyperformance run --bench raytrace
```
...
[ perf record: Captured and wrote 109.799 MB /root/homewroksbothsofthard/Project/subRay/results/baseline/perf.data (1596484 samples) ]
```

To avoid rerunning profiling, that existing raw profile was reused and copied in the VM to `Project/subRay/profiling/perf.data`.
...
Attempt 2 from inside `Project/subRay/profiling`:
...
Attempt 3 from inside `Project/subRay/profiling` using a short temporary output name:
...
- `Project/subRay/profiling/perf_report.txt` was created in the VM,
...
- `Project/subRay/profiling/perf.data` was created in the VM workspace by copying the successful baseline `perf.data`.
- After earlier failed attempts caused by VM serial-console truncation, a short-command workflow succeeded and created `Project/subRay/profiling/perf_report.txt` in the VM.
