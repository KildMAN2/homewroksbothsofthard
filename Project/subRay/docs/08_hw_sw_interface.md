...
(implemented in `Project/subRay/hw/rtl/`) and what remains proposed/future work.
...
### IMPLEMENTED (as simulated in `Project/subRay/hw/rtl/intersect_accel.sv`)
...
- Direct port-level control/status/data interface, exercised by a SystemVerilog
  testbench with a real-valued reference model (see `Project/subRay/hw/tb/`).
- ModelSim simulation with all checks passing (see `Project/subRay/hw/results/`).
