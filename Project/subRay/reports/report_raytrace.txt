...
- All claims are based on project artifacts under `Project/subRay/docs/`, `Project/subRay/results/`, `Project/subRay/profiling/`, and `Project/subRay/hw/results/`.
- Measured results are explicitly separated from estimates.
- No information in this report is invented.

Primary final outcome:
- Best software version selected: `Project/subRay/optimized/final/` (copied from Attempt 1).
...
- SystemVerilog RTL in `Project/subRay/hw/rtl/`.
- SystemVerilog testbenches in `Project/subRay/hw/tb/`.
...
Early baseline run summary (from `Project/subRay/reports/baseline_results.txt`):
- Mean: **26.8 s**
- Std dev: **5.7 s**
- Max: **48.7 s**
...
- `Project/subRay/profiling/flamegraph_original.svg`
- `Project/subRay/profiling/flamegraph_pyspy.svg`
...
- Correctness: `IDENTICAL=YES` (`Project/subRay/results/attempt1/correctness_report.txt`).

### Attempt 2
- Strategy: local alias/lookup hoisting for hot globals/helpers in `ray_colour()` and `bench_raytrace()`.
- Correctness: `IDENTICAL=YES` (`Project/subRay/results/attempt2_correctness_report.txt`).

### Attempt 3
- Strategy: inlined the per-light visibility check in the Lambert loop to reduce call/frame overhead.
- Correctness: `IDENTICAL=YES` (`Project/subRay/results/attempt3_correctness_report.txt`).
...
- `Project/subRay/optimized/final/` (copied from Attempt 1).
...
Documented outcomes:
- Attempt 1: `IDENTICAL=YES` with matching hash (`Project/subRay/results/attempt1/correctness_report.txt`).
- Attempt 2: `IDENTICAL=YES` with matching hash (`Project/subRay/results/attempt2_correctness_report.txt`).
- Attempt 3: `IDENTICAL=YES` with matching hash (`Project/subRay/results/attempt3_correctness_report.txt`).
...
- checker scripts under `Project/subRay/scripts/`
...
- Original: `Project/subRay/results/original_official.txt`
- Final: `Project/subRay/results/final_official.txt`
...
Artifacts:
- Baseline (non-fast): `Project/subRay/profiling/perf_stat_baseline.txt`
- Final (non-fast): `Project/subRay/profiling/perf_stat_final.txt`
...
Artifacts:
- Baseline: `Project/subRay/profiling/perf_report_suite_baseline.txt`
- Final: `Project/subRay/profiling/perf_report_suite_final.txt`
...
- Baseline: `Project/subRay/profiling/flamegraph_suite_baseline.svg`
- Final: `Project/subRay/profiling/flamegraph_suite_final.svg`

Authoritative matched profiling reports (same captures):
- Baseline: `Project/subRay/profiling/perf_report_suite_baseline.txt` (`_PyEval_EvalFrameDefault` approximately `23.50%`)
- Final: `Project/subRay/profiling/perf_report_suite_final.txt` (`_PyEval_EvalFrameDefault` approximately `27.39%`)
...
- `Project/subRay/profiling/flamegraph_pyspy_baseline.svg` and `Project/subRay/profiling/flamegraph_pyspy_final.svg` are preserved as supplemental historical evidence.
...
IMPLEMENTED:
- Q16.16 fixed-point RTL modules in `Project/subRay/hw/rtl/` (`fxp_sqrt.sv`, `sphere_intersect.sv`, `intersect_accel.sv`).
- Direct testbench port-level interface in `Project/subRay/hw/tb/`.

SIMULATED:
- ModelSim compile: 0 errors, 0 warnings.
- All `18/18` functional checks passed (`tb_fxp_sqrt`: 11, `tb_intersect_accel`: 7); evidence in `Project/subRay/hw/results/`.

ESTIMATED (not measured):
- `200 MHz` target clock, cycles/ray, throughput, Amdahl-based speedups, area, power, and bandwidth (see `Project/subRay/docs/09_hardware_performance.md`).

PROPOSED (not implemented):
- MMIO register map, DMA, input/output buffering, Python/C wrapper, driver, and real FPGA integration (see `Project/subRay/docs/08_hw_sw_interface.md`).
