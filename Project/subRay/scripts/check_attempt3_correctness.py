...
def main() -> int:
    ...
    result_dir = root_dir / "results" / "attempt3"
    result_dir.mkdir(parents=True, exist_ok=True)

    original_path = root_dir / "original" / "bm_raytrace" / "run_benchmark.py"
    optimized_path = root_dir / "optimized" / "attempt3" / "run_benchmark.py"

    original = load_module("raytrace_original", original_path)
    optimized = load_module("raytrace_attempt3", optimized_path)

    original_ppm = result_dir / "correctness_original.ppm"
    optimized_ppm = result_dir / "correctness_attempt3.ppm"
