...
`subPyflate/optimized/final/` = **a copy of Attempt 3**. Rationale:
...
- Source folder copied to `subPyflate/optimized/final/`.
...
Ran on Windows 11 with Python 3.12.10 using `subPyflate/scripts/windows_bench.py`
...
Populated from `subPyflate/results/original_official.txt` and
`subPyflate/results/final_official.txt`.
