...
All files are under `subPyflate/hw/rtl/` and `subPyflate/hw/tb/`.
...
by `subRay/hw/results/SIMULATION_RESULTS.txt`.

## 7. Simulation Commands

`subPyflate/hw/run_sim.sh` picks the first available simulator (ModelSim,
Icarus Verilog, or Verilator) and runs every testbench in sequence,
writing per-testbench logs into `subPyflate/hw/results/` and a summary
...
Implemented (RTL + testbench under `subPyflate/hw/`):
