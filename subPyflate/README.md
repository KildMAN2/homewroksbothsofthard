...
Benchmark #2 for the HWSW project. Companion to `subRay/`.
...
`subPyflate/scripts/run_profile.sh` (and by `run_baseline.sh` for the
...
```
bash subPyflate/scripts/script_pyflate.sh              # defaults to 'all'
...
```
python3 subPyflate/scripts/check_attempt1_correctness.py
python3 subPyflate/scripts/check_attempt2_correctness.py
python3 subPyflate/scripts/check_attempt3_correctness.py
python3 subPyflate/scripts/check_final_correctness.py
```

Hardware simulation:
```
bash subPyflate/hw/run_sim.sh                    # all three testbenches
bash subPyflate/hw/run_sim.sh tb_huffman_decoder # one testbench
```

## 8. Main Report

- Primary consolidated report: `subPyflate/reports/report_pyflate.md`
  (mirrored to `.txt`).
