#!/usr/bin/env python
"""
Optimization Attempt 1 for the pyflate benchmark.

Change vs original:
  * `HuffmanTable.find_next_symbol` now uses a canonical-Huffman lookup table
    indexed by `snoopbits(max_bits)`. Decoding a symbol becomes O(1) instead of
    O(#codes-in-table) per symbol, which is the profiled bottleneck of
    `decode_huffman_block`.

The lookup table is built lazily on first use, per direction (`reversed=True`
for gzip / LSB-first, `reversed=False` for bzip2 / MSB-first). If lookup fails
(e.g. an ill-formed table or, more importantly, end-of-stream where
`snoopbits(max_bits)` may raise before the last shorter code can be matched)
the code falls back to the original linear scan, preserving exact semantics.

Everything else in this file is byte-identical to the preserved original in
subPyflate/original/bm_pyflate/run_benchmark.py.

Copyright header from the original file is preserved below.
"""
...
if __name__ == '__main__':
    ...
    runner.metadata['description'] = "Pyflate benchmark (Attempt 1: canonical-Huffman LUT)"
    ...
    runner.bench_time_func('pyflate_attempt1', bench_pyflake, default_data)
