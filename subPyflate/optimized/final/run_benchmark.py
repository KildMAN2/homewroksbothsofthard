#!/usr/bin/env python
"""
Pyflate — FINAL software implementation (a copy of Attempt 3).

Selection basis: Attempt 3 layers all Attempt 1/2 optimizations plus hot-loop
attribute hoisting, so it is expected to be the fastest correct version. The
authoritative pick after VM measurement is recorded in
`subPyflate/docs/06_final_software.md`. If a different attempt turns out to be
strictly faster on the VM, this folder is re-copied from that attempt (the
attempts themselves are preserved untouched).
"""
...
if __name__ == '__main__':
    ...
    runner.metadata['description'] = "Pyflate benchmark (FINAL, based on Attempt 3)"
    ...
    runner.bench_time_func('pyflate_final', bench_pyflake, default_data)
