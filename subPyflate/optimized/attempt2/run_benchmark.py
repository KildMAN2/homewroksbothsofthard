#!/usr/bin/env python
"""
Optimization Attempt 2 for the pyflate benchmark.

Adds on top of Attempt 1:
  * `move_to_front(l, c)` uses `l.insert(0, l.pop(c))` — no intermediate list
    slice allocations, still O(len(l)) at the C level.
  * A precomputed `_INT2BYTE` table replaces `int2byte(i)` in the hot
    `favourites`/gzip-store paths.
  * `bwt_transform` builds the `base[]` array with a single-pass frequency
    count + cumulative sum instead of 256 `bytes.find` scans over the whole
    block. Semantics are byte-identical to the original.

Everything else is inherited from Attempt 1 (canonical Huffman LUT).
"""
...
if __name__ == '__main__':
    ...
    runner.metadata['description'] = "Pyflate benchmark (Attempt 2: LUT + MTF/BWT/int2byte)"
    ...
    runner.bench_time_func('pyflate_attempt2', bench_pyflake, default_data)
