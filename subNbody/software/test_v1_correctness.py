...
V1_PATH = ROOT / "run_benchmark_v1_scalar.py"
...
def main():
    v1 = load_module("nbody_v1", V1_PATH)

    baseline_bodies = clone_bodies(v1.BODIES)
    v1_bodies = clone_bodies(v1.BODIES)

    baseline_system = list(baseline_bodies.values())
    v1_system = list(v1_bodies.values())

    baseline_pairs = combinations_ref(baseline_system)
    v1_pairs = v1.combinations(v1_system)

    # Short correctness run, intentionally not a full benchmark.
    dt = 0.01
    steps = 100

    offset_momentum_original(baseline_bodies[v1.DEFAULT_REFERENCE], baseline_system)
    v1.offset_momentum(v1_bodies[v1.DEFAULT_REFERENCE], v1_system)

    e0_before = report_energy_original(baseline_system, baseline_pairs)
    e1_before = v1.report_energy(v1_system, v1_pairs)

    advance_original(dt, steps, baseline_system, baseline_pairs)
    v1.advance(dt, steps, v1_system, v1_pairs)

    e0_after = report_energy_original(baseline_system, baseline_pairs)
    e1_after = v1.report_energy(v1_system, v1_pairs)

    max_diff = max_state_diff(baseline_system, v1_system)
    e_before_diff = abs(e0_before - e1_before)
    e_after_diff = abs(e0_after - e1_after)
    ...
    print(f"energy_before_v1={e1_before:.15e}")
    print(f"energy_after_original={e0_after:.15e}")
    print(f"energy_after_v1={e1_after:.15e}")
