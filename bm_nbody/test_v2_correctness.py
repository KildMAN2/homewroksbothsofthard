...
V2_PATH = ROOT / "run_benchmark_v2_unroll.py"
...
def main():
    v2 = load_module("nbody_v2", V2_PATH)

    baseline_bodies = clone_bodies(v2.BODIES)
    v2_bodies = clone_bodies(v2.BODIES)

    baseline_system = list(baseline_bodies.values())
    v2_system = list(v2_bodies.values())

    baseline_pairs = combinations_ref(baseline_system)
    v2_pairs = v2.combinations(v2_system)

    # Short correctness run, intentionally not a full benchmark.
    dt = 0.01
    steps = 100

    offset_momentum_original(baseline_bodies[v2.DEFAULT_REFERENCE], baseline_system)
    v2.offset_momentum(v2_bodies[v2.DEFAULT_REFERENCE], v2_system)

    e0_before = report_energy_original(baseline_system, baseline_pairs)
    e2_before = v2.report_energy(v2_system, v2_pairs)

    advance_original(dt, steps, baseline_system, baseline_pairs)
    v2.advance(dt, steps, v2_system, v2_pairs)

    e0_after = report_energy_original(baseline_system, baseline_pairs)
    e2_after = v2.report_energy(v2_system, v2_pairs)

    max_diff = max_state_diff(baseline_system, v2_system)
    e_before_diff = abs(e0_before - e2_before)
    e_after_diff = abs(e0_after - e2_after)
    ...
    print(f"energy_before_v2={e2_before:.15e}")
    print(f"energy_after_original={e0_after:.15e}")
    print(f"energy_after_v2={e2_after:.15e}")
