...
- Original mean: `81.285 s` (± `0.198 s`) — `subRay/results/original_official.txt`
- Final (software-optimized) mean: `19.7062 s` (± `0.0133 s`) — `subRay/results/final_official.txt`
- Official software-only improvement already banked: `75.76%`

MEASURED (perf sampling, `subRay/profiling/perf_report.txt` and
...
`subRay/docs/06_hardware_candidate.md`.
...
Per `subRay/docs/06_hardware_candidate.md`, the accelerated fraction `P` is
...
ESTIMATE, carried from `subRay/docs/07_hardware_architecture.md` (RTL-latency
...
- ~23–25 cycles per ray for closest-hit against the current 7-sphere + 1-plane
  ...
  RTL actually simulated in `subRay/hw/rtl/` is the simpler iterative v1, which
...
- Consequence: **without batching, transfer overhead can equal or exceed
  ...
  `subRay/docs/06_hardware_candidate.md` and `08_hw_sw_interface.md` both call
...
- Multiple FP/fixed-point lanes (sqrt units especially) are DSP/LUT-heavy;
  `subRay/docs/07_hardware_architecture.md` already flags sqrt/div and
...
- The RTL actually implemented and simulated (`subRay/hw/rtl/`) uses Q16.16
  ...
  simulation (see `subRay/hw/results/`), not against the real Python/FP64
  benchmark output.
