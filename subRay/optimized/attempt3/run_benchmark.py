"""
Optimization Attempt 3 for the raytrace benchmark.

This version starts from Attempt 1 and applies one additional change in the
Lambert lighting path: inlining the per-light visibility check to reduce hot
...
def ray_colour(px, py, pz, dx, dy, dz, depth, spheres, lights):
    ...
    for sphere in spheres:
        ...
        t = sphere_intersection(px, py, pz, dx, dy, dz, cx, cy, cz, radius)
        if t is not None and t > -EPSILON:
    ...
    if best_kind == "sphere":
        ...
        normal_x, normal_y, normal_z = normalize(hit_x - cx, hit_y - cy, hit_z - cz)
    ...
    if SPECULAR_COEFFICIENT > 0.0:
        reflected_x, reflected_y, reflected_z = reflect(dx, dy, dz, normal_x, normal_y, normal_z)
        reflected_x, reflected_y, reflected_z = normalize(reflected_x, reflected_y, reflected_z)
        ...
        colour_r += SPECULAR_COEFFICIENT * reflected_r
        colour_g += SPECULAR_COEFFICIENT * reflected_g
        colour_b += SPECULAR_COEFFICIENT * reflected_b

    if LAMBERT_COEFFICIENT > 0.0:
        lambert_amount = 0.0
        for light_x, light_y, light_z in lights:
            light_dx, light_dy, light_dz = normalize(light_x - hit_x, light_y - hit_y, light_z - hit_z)
            is_visible = True
            for cx, cy, cz, radius, _, _, _ in spheres:
                t = sphere_intersection(hit_x, hit_y, hit_z, light_dx, light_dy, light_dz, cx, cy, cz, radius)
                if t is not None and t > EPSILON:
                    is_visible = False
                    break

            if is_visible:
                plane_t = halfspace_intersection(light_dx, light_dy, light_dz, 0.0, 1.0, 0.0)
                if plane_t is not None and plane_t > EPSILON:
                    is_visible = False
        ...
        if lambert_amount > 1.0:
            lambert_amount = 1.0
        scale = LAMBERT_COEFFICIENT * lambert_amount
        colour_r += scale * base_r
        colour_g += scale * base_g
        colour_b += scale * base_b

    if AMBIENT_COEFFICIENT > 0.0:
        colour_r += AMBIENT_COEFFICIENT * base_r
        colour_g += AMBIENT_COEFFICIENT * base_g
        colour_b += AMBIENT_COEFFICIENT * base_b
    ...
    for _ in range_it:
        ...
        for y in range(canvas.height):
            ...
            for x in range(canvas.width):
                ...
                ray_dx, ray_dy, ray_dz = normalize(ray_dx, ray_dy, ray_dz)
                colour_r, colour_g, colour_b = ray_colour(
...
if __name__ == "__main__":
    ...
    runner.metadata['description'] = "Simple raytracer attempt3"
    runner.metadata['raytrace_width'] = args.width
    runner.metadata['raytrace_height'] = args.height

    runner.bench_time_func('raytrace_attempt3', bench_raytrace,
