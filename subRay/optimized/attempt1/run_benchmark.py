"""
Optimization Attempt 1 for the raytrace benchmark.

This version keeps the benchmark behavior but rewrites the hot vector/point/ray
path to use scalar float locals and tuple-based scene data instead of repeatedly
allocating custom Python geometry objects in the inner loops.
"""
...
def ray_colour(px, py, pz, dx, dy, dz, depth, spheres, lights):
    ...
    if LAMBERT_COEFFICIENT > 0.0:
        ...
        for light_x, light_y, light_z in lights:
            ...
            if visible_light(hit_x, hit_y, hit_z, light_dx, light_dy, light_dz, spheres):
...
if __name__ == "__main__":
    ...
    runner.metadata['description'] = "Simple raytracer attempt1"
    runner.metadata['raytrace_width'] = args.width
    runner.metadata['raytrace_height'] = args.height

    runner.bench_time_func('raytrace_attempt1', bench_raytrace,
